...
Agent: `plan` | Model: Provider default (verify current model) | Timeout: `2m`
...
Agent: `plan` | Model: Provider default (verify current model) | Timeout: `5m`
...
Agent: `plan` | Model: Provider default (verify current model) | Timeout: `10m`
...
**Keep queries focused** — include only batch summaries (not full messages)
to stay within context limits. Full messages are available via Read tool if needed.

**Verify model names** — check `opencode-configuration` for current provider
model names before deployment. Model availability changes across providers.

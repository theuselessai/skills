...
description: >
  ...
  ad-hoc queries via the plan agent. Delivers final summary via intermediary-delivery
  skill. Uses opencode run for queries and end-of-session synthesis. Triggers on:
...
| `? <query>` | Fire query via `plan` agent for an explicit question |
...
1. Write `current_batch` to `meeting_minutes/<slug>/batch-NNN.json` with a
   ...
   ```bash
   timeout 2m opencode run \
     --agent plan \
     --format json \
     --dir /workspace \
     "$BATCH_SUMMARY_PROMPT"
   ```
   Model: Provider default (verify current model against `opencode-configuration`).
...
## Step 4: Query via plan Agent (? queries)

When a message starts with `? `, fire a query using the existing `plan` agent.
...
| Phase | Agent | Model |
|---|---|---|
| Queries (`?`) | `plan` | Provider default (verify current model) |
| Final synthesis | `plan` | Provider default (verify current model) |
| Batch summary | `plan` | Provider default (verify current model) |

> **Note:** Verify model names against `opencode-configuration` before use.
> Model availability and names change across providers.
...
This skill reuses the `plan` agent from `opencode-configuration`:

| Agent | Purpose | Tools |
|---|---|---|
| `plan` | Queries, synthesis, batch summaries | read, glob, grep |

Run the `opencode-configuration` skill to set up this agent.
...
2. **Queries fire only on explicit `?` prefix.** Never on passive messages.
...
7. **Ensure opencode-configuration has been run** before using this skill.

---

## References

- `references/prompts.md` — Prompt templates for batch summary, query, end synthesis
